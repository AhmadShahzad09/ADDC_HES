...
public class AppUtils {
    ...
    // Parses String to relevant Long value and Sets 0l for invalid string as default.
    ...
    public static Long getDateFromStringUTC(String dateStr) {
    	DateFormat formatter = new SimpleDateFormat("E MMM dd HH:mm:ss Z yyyy", Locale.US);
		try {
			Date date = (Date)formatter.parse(dateStr);
			return AppUtils.getTimeInSeconds(date.getTime());
		} catch (ParseException e) {
			e.printStackTrace();
		}
        return null;
