package com.minsait.oum.mdc.stream.heartbeat.data;
...
//	private static final int ELEMENTS_PER_PAGE = 50;
	...
	@Autowired
	private CallGroupDomain callGroupDomain;
	...
	public Call createCallMonitorEntry(Request request) {
		...
		if (!callOpt.isPresent()) {
			...
			result.setPid(requestId);
			setCommunication(request, result);
			result = callDomain.save(result);
		} else {
			result = callOpt.get();
			setCommunication(request, result);
		}

		Optional<CallGroup> callGroupOpt = callGroupDomain.findById(result.getIdGroup());

		if (!callGroupOpt.isPresent())
			callGroupDomain.save(createNewGroup(result));
	...
	private Call updateCallStatus(Call call, Status status, String message, boolean updateTasks) {
	...
	private Call updateCallAndTaskStatus(Call call, Status callStatus, String callGeneralError, Request request) {
		long finishTime = Calendar.getInstance().getTimeInMillis();
	...
	public List<Call> findCallByStatusFromTime(final String name, final Status status, final long from) {
		return callDomain.getByStatusAndFinishtimeGreaterThan(name, status.name(), from);
	}

	public List<Call> findCallByStatusFromTime(final Status status, final long from) {
		return callDomain.getByStatusAndFinishtimeGreaterThan(status.name(), from);
	}

	public void save(final List<Call> calls) {
		callDomain.saveAll(calls);
	}
