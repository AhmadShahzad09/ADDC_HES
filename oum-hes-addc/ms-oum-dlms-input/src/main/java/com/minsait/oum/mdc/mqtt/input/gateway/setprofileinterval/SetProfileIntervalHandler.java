...
@Slf4j
public class SetProfileIntervalHandler extends AbstractMessageHandlerProcessor {

	@Autowired
	private SetProfileIntervalAdapter adapter;
	...
	public void handleMessage(Message<?> message) {
		...
		this.processTelemetryCallToTaskListener(adapter.convert(message.getPayload().toString()),
				(request, call, device) -> {
					...
					if(StringUtils.isNotEmpty(device.getInstrumentationProfile()))
