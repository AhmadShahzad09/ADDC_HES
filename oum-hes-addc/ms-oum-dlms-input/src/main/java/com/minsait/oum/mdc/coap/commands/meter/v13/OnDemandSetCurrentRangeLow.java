...
@Slf4j
@Service("OnDemandSetCurrentRangeLow_V13")
public class OnDemandSetCurrentRangeLow extends AbstractOnDemand {
...
//			@Override
//			public void run() {
//				executeCommand(buildCommand(id, orders), AppConstants.OnDemand.COAP_PATH.SET_CURRENT_RANGE_LOW, ipPort);
//			}
//		}).start();
//	}
//
...
//		commandVO.setType(AppConstants.OnDemand.COMMAND_TYPE.CURRENT_RANGE_LOW);
//
...
//	}
	...
	protected String getCoapPath() {
		return AppConstants.OnDemand.COAP_PATH.SET_CURRENT_RANGE_LOW;
	}

	@Override
	protected void customizeCommand(OnDemandCommandVO commandVO, OnDemandOrder orders) {
		commandVO.setType(AppConstants.OnDemand.COMMAND_TYPE.CURRENT_RANGE_LOW);
		commandVO.setThreshold(orders.getParameters().get("CurrentRangeLowThreshold(A)"));
	}
}
